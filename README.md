...
| `yarn promo` | 宣传用的动画短片 | **不录屏**，逐帧画出来再编码：虚拟时钟保证帧即时间，输出 16:9 与 9:16 两版。适合做纯动画的片头/转场，见下。 |
...
`{场景}.mp4` 是原始录屏，**不是中间产物**——`concat.txt` 指向它们，删掉就没法重出总片。

### promo

`yarn video` 录的是真实界面，适合讲「怎么用」；`yarn promo` 画的是纯动画，适合讲「结果有多重」。它不驱动应用、不截图，而是让一个舞台页面按帧号把画面画出来：

```powershell
yarn promo                     # 全部画幅，输出 smoke-out/promo/
yarn promo --aspect 9x16       # 只出竖版
yarn promo --beat explosion    # 指定段落
yarn promo:diag                # 只出一帧带方向标记的 PNG，用来核对旋转
```

为什么值得单独做一套，而不是录屏再剪：

- **帧号就是时间。** 舞台暴露一个纯函数 `window.frame(f)`，第 f 帧的画面只由 f 决定。录屏做不到这一点：抓帧循环永远追不上设定的帧率，只能事后用 `setpts` 把帧数拉回真实时长（`make-video.cjs` 就是这么做的，每段还要报 `drift`）。这里是反过来——先定时间，再画。成品恒为 `帧数 ÷ 帧率`，实测 `drift` 是 `+0.000s`。
- **同一帧重画两次得到同一张图。** 随机量走固定种子的 `mulberry32`，不用 `Math.random`；改一行文案重渲，其余构图不会跟着漂。
- **没有缩放。** 离屏窗口会被静默钳到显示器工作区（这台机器是 1920×1032，即 1080 减掉任务栏），所以舞台直接按 1920×1032 构图，再补边到 1920×1080：编码器收到的就是画出来的，中间没有放大。
- **竖版靠旋转拿原生像素。** 竖版如果按 580×1032 画再放大到 1080×1920，字会糊；所以竖版是**横着画、编码时转回来**（`transpose=1`），两个画幅都是原生分辨率。画布变换必须和编码器的旋转方向抵消，方向是量出来的不是推出来的——`yarn promo:diag` 会输出一张带 `TL`/`BR` 标记和顶边红条的 PNG，直接看红条落在哪条边即可。

几个踩过的坑写在 `build/stage/stage.js` 的注释里：**同一进程里销毁离屏窗口会让 GPU 进程失效**（`GPU state invalid after WaitForGetOffsetInRange`），之后再开窗口一律 `ERR_FAILED`，所以一个段落只开一个窗口、两个画幅复用；扩散动画的方向**不能由打印位置推出**，那样 96 个字会挤成左边一条竖线（实测 x 只覆盖 80–285 / 1920），必须均匀分配角度。

产物：`{段落}-16x9.mp4`、`{段落}-9x16.mp4`。当前实现的是「信息爆炸」这一段（5.000s）：终端刷 `finding` → 溢出 → 扩散 → 回收成一个数 `389`。
